...
import menus.quiz_func.graph_generation as ggen
...
def createClearBtn(frame : ttk.Frame, quiz : qobj.Quiz, canv : tk.Canvas,
    ...
    clear_btn = ttk.Button(frame, text = 'Clear selections', takefocus=False, command = lambda : qnav.clearGraphSelections(quiz, canv, sel_v, sel_e, sol_txt, sol_btn_txt, marks_txt))
...
def drawQuizNumber(app : ttk.Window, frame : ttk.Frame, quiz : qobj.Quiz):
    ...
    question_number.pack(side = 'right',anchor='se',padx = GUI_PADDING + INNER_PADDING, pady = INNER_PADDING)
...
def drawQuestionNavBtn(app : ttk.Window, quiz : qobj.Quiz, frame : ttk.Frame):
    ...
    next_text = tk.StringVar(value = '>')

    if quiz.current_question == quiz.num_questions:
            next_text.set(value = 'End quiz')

    nextq_btn = ttk.Button(master = frame, textvariable=next_text, command = lambda : qnav.nextQuestion(app, quiz, next_text), takefocus=False)
...
def createGraphGui(app : ttk.Window, graph : gobj.Graph, right_frame : ttk.Frame, canv : tk.Canvas, quiz : qobj.Quiz, canv_frame : tk.Canvas):
    ...
    #back button
    back_btn = ttk.Button(master = top_ui_frame, text="Back to main menu", takefocus=False, command = lambda : st.gotoMainMenu(app))
    back_btn.pack(side='left', anchor='w', padx = GUI_PADDING + INNER_PADDING)
